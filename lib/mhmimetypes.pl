...
##-----------------------------------------------------------------------
    ...
    'sgl:StarWriter global document',
...
sub write_attachment {
    ...
    my ($fh, $tmpfile) =
        file_temp($ext . 'XXXXXXXXXX', $pathname, '.' . $ext);
    binmode($fh);
    print $fh $$sref;
    close($fh);
