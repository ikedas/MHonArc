...
# URL attributes
my $UAttr =
      q/\b(?:action|background|cite|classid|codebase|data|datasrc|/
...
BASEURL: {
    ...
    if ($nofont) {
        ...
        for ($i = 0; $$data =~ s/\b(?:style|class)\s*=\s*"[^"]*"//gio; ++$i) {
            return bad_html_reject("Nested style|class attributes")
                if $i > 0;
        }
        for ($i = 0; $$data =~ s/\b(?:style|class)\s*=\s*'[^']*'//gio; ++$i) {
            return bad_html_reject("Nested style|class attributes")
...
sub resolve_cid {
    ...
    if (defined($decodefunc) && defined(&$decodefunc)) {
        ...
        $filename = mhonarc::write_attachment($ctype, \$data,
            {'-dirpath' => $attachdir});
    } else {
        $filename = mhonarc::write_attachment($ctype, $href->{'body'},
            {'-dirpath' => $attachdir});
    }
